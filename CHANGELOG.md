...
- Users can create and manage groups, members, and model grants through the new Groups pages; admins manage group coin policies. (#41)
- Admins manage database-backed auto-join rules per group, with matching login claims keeping memberships synchronized. (#41)
- Models support configurable early-access acknowledgements and badges. (#41)
- Models support end dates that hide and disable expired entries. (#41)
- Model detail pages show when a model was first seen. (#41)

### Security

- Upstream endpoint API keys are encrypted at rest with `app.encryption_key`. (#41)
- Model endpoint probes pin validated DNS resolutions to prevent DNS rebinding. (#41)
- `/metrics/debug` supports a separate bearer token via `api.prometheus.debug_token`. (#41)
- The config editor rejects structurally invalid version 3 configurations before saving. (#41)
- Gravatar hashing works on FIPS-enabled Python builds. (#41)

### Changed

- Database and app-context diagnostics can be disabled with `app.diagnostics: false`. (#41)
- API and web-chat paths use one shared cost calculation. (#41)
- Admin user, project, and group-member pagination uses leaner count queries. (#41)
- Obsolete client-blueprint and unused config and balance helpers are removed. (#41)
- The implicit `default` group is removed; users without a group pool continue to use `defaults.tokens`. (#41)
- Project ownership transfers use a Change Owner dialog and require the new owner to already be a manager. (#41)
- Non-serving CLI commands no longer start background workers. (#41)
- CI enforces Python, YAML, and Helm linting. (#41)
- **Breaking:** Config version 3 removes user, project, client, group, and auto-join sections from config and Helm values; migrate old values files. (#41)
- **Breaking:** Model access is database-managed: unowned models are public; owned models are limited to their owner and granted groups. Back up the database and assign owners to restricted models when upgrading. (#41)
- **Breaking:** Project settings and per-user coin pools move from config to database-backed UI, preserving existing database values. (#41)
- Project detail pages use the profile-card and tabbed layout. (#41)
- Native browser dialogs are replaced with accessible application modals. (#41)
- Projects appear between Profile and Models in navigation. (#41)
- Project and admin-user tables expose usage and coin columns, newest-first last-use sorting, and inline admin actions. (#41)
- Project and user settings open from inline edit buttons with role-aware access. (#41)
- Deactivated projects remain visible to their administrators and managers. (#41)
- Profile and project pages show the next refill time. (#41)

### Fixed

- Version 3 configuration examples and access-control documentation match runtime behavior. (#41)
- Tests use their isolated SQLite database instead of the developer database. (#41)
- Data tables consistently render cell borders. (#41)
- Model endpoint probes reject unsafe URLs, private addresses, and redirects. (#41)
- Request bodies are capped before oversized uploads are buffered. (#41)
- Models.dev knowledge cutoffs are normalized to `YYYY-MM`. (#41)
- Config-editor saves tolerate unwritable backup paths. (#41)
- Resetting coins uses the latest configured maximum. (#41)
- Clearing Max Coins removes the entity pool and restores inherited limits. (#41)
- Project manager ownership and removal actions handle quoted names correctly. (#41)
- Tests and dependency floors support OpenAI 3.x and `httpx2`. (#41)
