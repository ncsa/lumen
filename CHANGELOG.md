...
- Extended the connection-pool leak diagnostics so a `/metrics/debug` capture is self-contained. A production leak was confirmed but could not be attributed from the previous output: successive captures showed one checkout aging past 70 minutes on `api.list_models` while the thread that acquired it sat idle in the a2wsgi worker pool, and `pg_stat_activity` showed nothing because `idle_in_transaction_session_timeout` had already reaped the orphaned backend. Three additions:
  - A `=== DB pool status ===` section reporting the pool's own `size`/`checked_in`/`checked_out`/`overflow`/`max_overflow`, so a reported checkout can be confirmed against the pool rather than correlated against `pg_stat_activity` by hand.
  - A `=== what retains those checkouts ===` section that walks the reference graph from the pool's own record to whatever holds it, reporting one hop per line (`Connection` → `RootTransaction` → `SessionTransaction` → `Session`) plus whether the DBAPI connection is already dead. The checkout stack names where a connection was *acquired*, which this leak proved is not where it is *retained*. Type names only — never object reprs, since a Session's identity map holds API key hashes and user rows and this output is served over HTTP.
  - Stale tracker entries are now identified instead of counted as leaks. Entries are keyed by `id(connection_record)` and pruned only by the check-in event, so a record dropped without one left an entry that aged forever, pegging the `{state="stranded"}` gauge and keeping the watchdog armed. Each entry now holds a weakref to its record; entries whose record is gone are excluded from `stranded_count()` and marked `STALE-TRACKER-ENTRY`.

- Fixed a permanent DB connection-pool leak affecting any endpoint. Production captures showed connections stranded for up to six hours — `api.list_models` and `api.chat_completions`, all acquired in the API key lookup — with their WSGI threads long since idle and the retainer chain ending at a live `SessionTransaction`. Flask-SQLAlchemy's app-context teardown calls an unguarded `db.session.remove()`, which closes the session (rolling back) *before* clearing its registry, and `SessionTransaction.close()` detaches the transaction from the session *before* closing its connections. So a rollback that raises leaves the connection checked out with no handle left on it, while the still-registered session — the registry is a plain dict keyed by `id(app_ctx)` — pins it for the life of the process; Flask's teardown loop has no `except`, so the error escaped `AppContext.pop()` and was never logged. `create_app` now registers its own `teardown_appcontext` handler ahead of Flask-SQLAlchemy's that logs the failure with its traceback and clears the registry, releasing the last strong reference so SQLAlchemy's own finalizer returns the connection to the pool.
- `/metrics/debug` now reports how many sessions are still registered, and whether the Flask app context behind each outstanding checkout is still alive or has been collected — distinguishing a teardown that never ran from one that ran and leaked anyway.
- `/metrics/debug` now opens with a `=== deployment ===` section so a capture is judgeable without hunting through configs: hostname and app version, worker processes x replicas (with the `WEB_CONCURRENCY`/`LUMEN_REPLICAS` env vars they came from), the a2wsgi thread pool size (`LUMEN_WSGI_WORKERS`) plus how many WSGI threads have actually spawned, the resolved engine options (`pool_size`/`max_overflow`/`pool_timeout`/`pool_recycle`), the server's live `max_connections`, and the total pool budget across all processes against it. `max_connections` is queried on a throwaway unpooled connection, so it still answers while this process's own pool is exhausted — which is when the endpoint gets used.
- The context probes now record push provenance: every `AppContext.push` stores the pushing thread, time and stack (weak-keyed per live context), an `ambient-context-at-start` anomaly reports who pushed the leftover context plus which Flask app object it belongs to, and a pop on a different thread than the push is recorded as `cross-thread-pop` — the token reset cannot take effect outside the pushing thread's contextvars context, which leaves that thread poisoned with the context current at depth 0, matching a production capture (`depth=0` ambient context on `WSGI_0`).
- Closed the context probes' three blind spots after a capture showed 15 leaks with every probe silent (one never-torn-down session per worker thread — the poisoned-thread signature). The middleware's close()-time baseline comparison masked exactly that case, so it now also reports — once per context — an app context *already current when a request starts* (a poisoned thread); it checks the exception path too, where a raising request produces no body and the close()-time check never runs. `AppContext.pop`/`RequestContext.pop` raising is now recorded as an anomaly (a `RequestContext.pop` that fails resets the request contextvar but silently never pops the app context — teardown never runs with nothing logged). Anomalies now carry the thread name, and the scope report counts how many ring teardowns found no session — a share tracking the leak rate would mean teardowns run under a different key than the session's.
- Added probes that catch an unbalanced Flask app-context push/pop in the act. The instrumented scope report confirmed leaked sessions belong to app contexts whose teardown *never ran* even though the context was collected within seconds — the signature of `AppContext.pop()` skipping teardown when the context was pushed more than once. `AppContext.push`/`pop` are now wrapped (installed once per process from `create_app`) to log a WARNING with the offending stack whenever a context is pushed while already pushed, or popped at depth > 1 (which is exactly when Flask skips teardown). The metrics WSGI middleware additionally verifies, after the response body's `close()` — the last moment a request runs code — that the current app context is back to its request-start baseline, and names the endpoint that left one behind. Every catch (double push, skipped teardown, leftover context) is also kept in a ring buffer and served in a `/metrics/debug` `=== app-context anomalies ===` section — keyed by context id so it matches the scope report's registry keys — so a capture is self-contained rather than depending on the warning still being in the log. The `/metrics/debug` scope report also gives a teardown verdict for registered sessions holding no connection, instead of only for keys with outstanding checkouts.
- `/metrics/debug` now answers directly whether app-context teardown ever ran for a leaked checkout. A post-fix production capture disproved the failed-rollback theory: leaked sessions stayed registered with nothing in the log, meaning `db.session.remove()` was never called under their registry key at all — and a collected app context does not prove teardown ran, only that the context was garbage collected (a context abandoned without `pop()` is collected all the same; the `app_ctx=` field's earlier interpretation was too generous). Every checkout now records its scope key (`id(app_ctx)`, the scoped-session registry key), the teardown handler ring-buffers the scope key of every teardown it runs, and a new `=== scope keys ===` section cross-references the three: a stranded checkout whose key is still registered and shows `teardown=NEVER` was created under an app context that never went through teardown — e.g. a request that reused a stale context left pushed on its a2wsgi worker thread, which is also visible as two checkouts from different times sharing one scope key.
- The `/v1/chat/completions` streaming generator now releases its DB session before emitting the error events on the exception path, and in a `finally` covering the client-disconnect `GeneratorExit` path where `record_aborted_request` checks a connection back out. Billing re-acquires a connection mid-generator, so an error raised after it left the connection held across two `yield`s that a client that has gone away never lets finish — the discipline already documented in `/chat/stream`.
