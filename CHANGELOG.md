...
- **Upstream endpoint API keys are now encrypted at rest.** The `model_endpoints.api_key` column stores Fernet ciphertext keyed from `app.encryption_key` instead of plaintext (migration `z5c6d7e8f9a0` widens the column and encrypts existing rows); the ORM encrypts/decrypts transparently, so a database dump no longer exposes upstream provider credentials. Rotating `encryption_key` makes the stored ciphertext unreadable until the next startup re-syncs the keys from `config.yaml`.
- Model sync endpoint probes now pin the DNS resolution used for the SSRF check: the hostname is resolved and validated once and the probe connects to that exact IP (with TLS still verified against the hostname), closing the DNS-rebinding window between validation and fetch. Hostnames that do not resolve are now rejected outright.
- `/metrics/debug` can require its own bearer token: set `api.prometheus.debug_token` to fence off the thread dumps and deployment details behind a stronger secret than the scrape token (chart value `api.prometheus.debugToken`). Unset, it keeps using the scrape token.
- The admin config editor validates the config's structure before writing it to disk: a save missing `version: 3`, a model `name`, the per-million costs, or an endpoint `url`/`api_key` is rejected with a 400 naming the problem instead of breaking the next model sync or startup.
- All Gravatar MD5 calls now pass `usedforsecurity=False`, so they work on FIPS-enabled Python builds.

### Changed

- The stack-capturing DB/context diagnostics (pool checkout tracking and app-context push/pop probes behind `/metrics/debug`) can be disabled with `app.diagnostics: false` (chart value `config.diagnostics`; restart required) for deployments that don't want the per-request capture overhead. Default unchanged (enabled).
- Cost calculation is centralized in `lumen.services.cost.calculate_cost` — the API chat/completions paths and the web-chat stream previously each computed the same formula inline.
- The paginated admin users, projects, and group members endpoints count rows with a lean query on the base table instead of re-running the full joined/sorted query inside a subquery.
- Removed dead code: the unused `get_coin_balance` helper, the leftover `lumen/blueprints/clients/` directory (renamed to projects long ago), and the `_RESTART_REQUIRED` alias in the config watcher.
...
- **Breaking (Lumen 2.0): config version 3 is required.** The app refuses to start unless `config.yaml` declares `version: 3`, and the config watcher skips hot reloads of older files. The `users:`, `projects:`, `clients:`, and `groups:` sections were removed: groups, memberships, and coin pools are managed in the database (existing rows keep working; group and model management dialogs are planned), and OAuth auto-assignment rules are managed in the database on each group's Rules tab — there is no `group_rules:` config section (`app.dev_user.groups` still works for dev login). The admin config editor's Groups and Users sections are gone. The Helm chart emits version 3 and its `groups`/`users` values were removed; old values files fail schema validation and must be migrated.
