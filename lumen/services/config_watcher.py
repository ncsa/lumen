...
CONFIG_VERSION_ERROR = (
    ...
    "rules into the database (managed on each group's Rules tab; the config file "
    "has no group_rules: section)."
)
...
def validate_config_structure(data) -> list:
    ...
    Returns a list of human-readable error strings (empty when valid). Covers
    the keys :func:`sync_models_from_yaml` accesses unconditionally — a config
    missing one of them passes the version check but raises KeyError on the
    next sync, so it must be rejected before it reaches config.yaml.
    """
    ...
    for i, model in enumerate(models):
        ...
        if not name or not isinstance(name, str):
            errors.append(f"models[{i}]: missing required key 'name'")
        ...
        if not isinstance(endpoints, list):
            errors.append(f"{label}: 'endpoints' must be a list")
            continue
        for j, ep in enumerate(endpoints):
            if not isinstance(ep, dict):
                errors.append(f"{label}: endpoints[{j}] must be a mapping")
                continue
            if not ep.get("url") or not isinstance(ep.get("url"), str):
...
KNOWN_APP_KEYS = frozenset({
    ...
    "diagnostics", "email_themes", "encryption_key", "github_url",
    "graylist_default_notice", "logs", "name", "secret_key", "tagline", "theme",
})
